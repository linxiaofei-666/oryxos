...
class OrganizationsMigrationTest {

  @TempDir Path tempDir;

  @Test
  void createsOrganizationsAndAddsTeamsOrgIdIdempotently() throws Exception {
    SQLiteDataSource dataSource = dataSource("legacy-orgs.db");
    execute(
        dataSource,
        "CREATE TABLE teams ("
    ...
    assertThat(columns(dataSource, "organizations"))
        .containsExactlyInAnyOrder("org_id", "display_name", "created_at", "updated_at");
    assertThat(columns(dataSource, "teams")).contains("org_id");
  }
  ...
  private static void migrate(SQLiteDataSource dataSource) throws Exception {
    try (Connection connection = dataSource.getConnection()) {
      new OrganizationsMigration().migrate(connection);
    }
  }
