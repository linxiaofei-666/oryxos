...
class OrganizationsParentMigrationTest {

  @TempDir Path tempDir;

  @Test
  void addsParentOrgIdIdempotently() throws Exception {
    SQLiteDataSource dataSource = dataSource("legacy-org-parent.db");
    execute(
        dataSource,
        ...
        "INSERT INTO organizations (org_id, display_name, created_at, updated_at)"
            + " VALUES ('acme', 'Acme', '2026-08-01T00:00:00Z', '2026-08-01T00:00:00Z')");

    migrate(dataSource);
    migrate(dataSource);

    assertThat(columns(dataSource, "organizations"))
        .containsExactlyInAnyOrder(
            "org_id", "display_name", "created_at", "updated_at", "parent_org_id");
  }
  ...
  private static void migrate(SQLiteDataSource dataSource) throws Exception {
    try (Connection connection = dataSource.getConnection()) {
      new OrganizationsParentMigration().migrate(connection);
    }
  }
